...
const MORALIS_API_KEY = process.env.MORALIS_API_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJub25jZSI6IjI4NWZhOGE1LTBmYWItNDkxOC1hMmE3LTk0ZjFlNmJiMjI2ZSIsIm9yZ0lkIjoiNDgxOTA3IiwidXNlcklkIjoiNDk1NzgwIiwidHlwZUlkIjoiZmQzZGMwMjItM2VjOC00MjM5LWEzZjUtNTUxMTVjNGRhMjVjIiwidHlwZSI6IlBST0pFQ1QiLCJpYXQiOjE3NjM0MjIzNzksImV4cCI6NDkxOTE4MjM3OX0.BdOpy-5ApiiwAMM7TDRtY-knkLRGWUsHLf6B7y_nb5k';
...
async function processDepositTransaction(wallet, amount, txid, network) {
  try {
    ...
    if (depositError) {
      ...
      if (depositError.code === '23505') { // Код ошибки уникального ограничения в PostgreSQL
    ...
    if (userError) {
      // Откатываем вставку депозита при ошибке
    ...
    // Обновляем баланс пользователя (без updated_at для совместимости)
    ...
    if (updateError) {
      // Откатываем вставку депозита при ошибке обновления баланса
      await supabase.from('deposit_transactions').delete().eq('id', newDeposit.id);
    ...
    // Обновляем статус депозита на 'processed'
    await supabase
      .from('deposit_transactions')
      .update({ status: 'processed' })
      .eq('id', newDeposit.id);
